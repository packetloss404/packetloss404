<p align="center">
  <img src="packetloss404.jpg" alt="packetloss404 — agentic battle station" width="100%"/>
</p>

```
packetloss404@void:~$ whoami
> Ian Walmsley — Agentic Engineer
> shipping agents that ship agents
```

## currently building

> Shipping across a stack of agent-driven tools — **PacketADE** stitches Claude Code + Codex CLI into a single multi-pane workspace, **packetcode** is the keyboard-first terminal coding agent behind it, **vibesocial** and **vibebbs** experiment with community infra for the streamer / BBS crowd, **taskloom** is the lightweight task engine holding it all together, and **mc-server-bot** keeps the self-hosted Minecraft fleet running.
>
> [PacketADE](https://github.com/packetloss404/packetade) · [packetcode](https://github.com/packetloss404/packetcode) · [vibesocial](https://github.com/packetloss404/vibesocial) · [taskloom](https://github.com/packetloss404/taskloom) · [mc-server-bot](https://github.com/packetloss404/mc-server-bot) · [vibebbs](https://github.com/packetloss404/vibebbs)

## projects

<table>
<tr>
  <td width="33%" align="center"><a href="https://github.com/packetloss404/packetade"><img src="cards/packetade.svg" alt="PacketADE" width="100%"/></a></td>
  <td width="33%" align="center"><a href="https://github.com/packetloss404/packetcode"><img src="cards/packetcode.svg" alt="packetcode" width="100%"/></a></td>
  <td width="33%" align="center"><a href="https://creditquest.packetloss404.com"><img src="cards/creditquest.svg" alt="Credit Quest" width="100%"/></a></td>
</tr>
</table>

## latest from the lab

<p align="center">
<a href="https://www.youtube.com/watch?v=tB2kS0MhOb0"><img src="https://i1.ytimg.com/vi/tB2kS0MhOb0/hqdefault.jpg" alt="Vellum — Built with Claude Opus 4.7 (Cerebral Valley Hackathon 2026)" width="360"/></a><br><sub>Vellum — Built with Claude Opus 4.7 (Cerebral Valley Hackathon 2026)</sub>
</p>

## recent shipments

- `packetcraft` — [Persist chat history, events & media services (Batch 3)](https://github.com/packetloss404/packetcraft/commit/e152eaf2cb948b7d0c90d5d15cffbf7f29554361) <sub>· 2026-06-02</sub>
- `packetcraft` — [Retire the Godot native client; revert to Paper + Fabric over the REST sidecar](https://github.com/packetloss404/packetcraft/commit/9842de111583f83a1e68aacb37170339643e5d0c) <sub>· 2026-06-02</sub>
- `vibetotext` — [Repo hygiene + scrub original-author attribution (takeover cleanup)](https://github.com/packetloss404/vibetotext/commit/6de2158b58fa8c17fd1f9c712135b7f99f976865) <sub>· 2026-06-02</sub>
- `vibetotext` — [Remove the "Don't Anger the AI" game; extracted to packetloss404/vibegalaxy](https://github.com/packetloss404/vibetotext/commit/a4a72c0c54e75b6fdbe1b2009a0d6e055ef04287) <sub>· 2026-06-02</sub>
- `vibetotext` — [Remove byte-identical duplicate source trees](https://github.com/packetloss404/vibetotext/commit/b69fe1f42009b9a3cff336200a3af2055ecfc7e5) <sub>· 2026-06-02</sub>

---

<p align="center">
  <a href="https://www.youtube.com/@packetloss404"><code>[ y ] youtube</code></a> ·
  <a href="https://discord.gg/eygyg7pQ"><code>[ d ] discord</code></a> ·
  <a href="https://creditquest.packetloss404.com"><code>[ c ] credit quest</code></a>
</p>

<p align="center"><sub>last refresh: <code>2026-06-03</code> · auto-built from <code>templates/README.template.md</code></sub></p>
